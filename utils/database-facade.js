...
const facade = module.exports = {
  ...
  queries: {
    ...
    getRegistration: 'SELECT registration.id AS id, userid AS userId, registration.registrationnumber AS registrationNumber, roompreference AS roomPreference, earlyarrival AS earlyArrival, latedeparture AS lateDeparture, buytshirt AS buyTshirt, buyhoodie AS buyHoodie, tshirtsize AS tshirtSize, hoodiesize AS hoodieSize, registration.timestamp AS timestamp, paymentdeadline AS paymentDeadline, isadminapproved AS isAdminApproved, receivedinsidespot AS receivedInsideSpot, receivedoutsidespot AS receivedOutsideSpot, refundchoice AS refundChoice, donationamount AS donationAmount, COALESCE(SUM(amount), 0) AS paidAmount FROM registration LEFT JOIN payment ON (registration.id = payment.registrationid) WHERE userid = ? GROUP BY registration.id',
    ...
    setRefundChoice: 'UPDATE registration SET refundchoice = ? WHERE userid = ?',
